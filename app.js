...
let selected = null, lastMove = null, channel = null, privateRoom = false, computerMode = false, thinking = false, difficulty = "medium", color = null, host = false, started = false, undoState = null;
...
async function clickSquare(square) {
  ...
  if (selected) {
    ...
    if (move) { selected = null; lastMove = { from: move.from, to: move.to }; if (computerMode) undoState = previous; if (privateRoom) await channel.send({ type:"broadcast", event:"move", payload:{ fen:chess.fen(), from:move.from, to:move.to } }); draw(); if (computerMode && !chess.game_over()) computerMove(); return; }
  }
...
async function joinPrivate(code, isHost) { privateRoom=true; computerMode=false; host=isHost; started=false;color=null; codeDisplay.textContent=code; $("lobbyCodeDisplay").textContent=code; $("undoBtn").classList.add("hidden"); show(lobby); channel=supabaseClient.channel(`chess-room-${code}`,{config:{presence:{key:id},broadcast:{self:false}}}); channel.on("presence",{event:"sync"},updateLobby).on("presence",{event:"leave"},({key})=>{if(key!==id&&started){alert("Your opponent left the game.");leavePrivate();}else updateLobby();}).on("broadcast",{event:"player-left"},()=>{if(started){alert("Your opponent left the game.");leavePrivate();}}).on("broadcast",{event:"start"},({payload})=>{if(payload.colors[id]){color=payload.colors[id];enterGame();}}).on("broadcast",{event:"move"},({payload})=>{chess.load(payload.fen);lastMove=payload.from?{from:payload.from,to:payload.to}:null;draw();}).on("broadcast",{event:"new-game"},()=>{started=false;color=null;chess.reset();lastMove=null;show(lobby);channel.track({playerId:id,color:null});updateLobby();}); channel.subscribe(async(state)=>{if(state==="SUBSCRIBED"){if(players().length>=2){lobbyError.textContent="This room already has two players.";await leavePrivate();return;}await channel.track({playerId:id,color:null});updateLobby();}}); }
